...
namespace Ass {
    enum GestureType {
        ...
        int MyForm::CountFingers(const cv::Mat& roi) {
            ...
            static std::vector<int> xHistory; // เก็บประวัติตำแหน่งแกน X ของจุดศูนย์กลางมือ
            const int MAX_HISTORY = 17;
            const int MOTION_HISTORY = 10; // เก็บประวัติการขยับ 10 เฟรมล่าสุด
        ...
        // --- เพิ่ม: คำนวณ Centroid และการเคลื่อนที่แนวนอน (สำหรับตรวจโบกมือ) ---
        ...
        int validDefects = 0;
        double maxDistanceBetweenFingers = 0; // เพิ่มตัวแปรเช็คระยะห่าง
        ...
        // --- ตรรกะแยกท่าทาง (เพิ่มเงื่อนไขโบกมือ) ---
        int rawResult = GESTURE_NONE;
        int fingerCount = validDefects + 1;

        if (fingerCount >= 5) {
            // ถ้าชู 5 นิ้ว และมีการขยับแกน X มากกว่า 30 พิกเซล (ปรับตัวเลขนี้ได้)
            if (movement > 30) {
            ...
            while (isRunning) {
                ...
                if (hasROI) {
                    ...
                    if (x2 > x1 && y2 > y1) {
                        ...
                        // 2. แปลงค่าตัวเลขเป็นข้อความด้วย switch-case
                        std::string label = "";
                        switch (result) {
                        ...
                        case GESTURE_loveu: label = "Love U"; break; // ท่าใหม่ที่เราเพิ่ม
